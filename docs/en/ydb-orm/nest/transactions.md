...
In NestJS applications, `YdbTransactionManager` is automatically available via DI when `YdbCoreModule.forRootAsync()` is imported.
...
If you don't use NestJS, see the [standalone transactions](../transactions.md) guide with `new YdbTransactionManager(executor)`.
