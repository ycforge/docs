...
Пакет устанавливает бинарь `yorm`:
...
В TTY команда `yorm entity:create <name>` запускает мастер генерации сущности:
...
### Mermaid ER-диаграмма: entity:diagram

Read-only команда рендерит те же канонические метаданные (тот же источник, что у `metadata:dump` — `buildMetadataDump`) в Mermaid ER-диаграмму — **без подключения к БД**: ни драйвера, ни executor'а, ни DDL. Валидные метаданные обязательны: любая ошибка конфигурации роняет команду до первого байта вывода.
...
- связи one-to-many / many-to-one по одной join-колонке дают одну линию `||--o{`; однонаправленный one-to-many рисуется от родителя; one-to-one — линией `||--o|`; FK-колонки помечены `FK`;
...
- детерминизм: порядок блоков и линий стабилен и не зависит от порядка входного списка — повторный запуск даёт побайтово одинаковый вывод;
...
Для внешних инструментов доступен программный API: `buildEntityDiagram(entities)` и `writeDiagramFile(path, diagram)` экспортируются из пакета.
...
Файл `./yorm.config.ts` (или `.mts`/`.mjs`/`.js`; также ищется в `./src/`). Поддерживается и legacy-имя `./ydb-orm.config.*` — при наличии обоих приоритет у `yorm.config.*`:
